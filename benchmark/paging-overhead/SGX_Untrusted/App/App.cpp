...
sgx_status_t load_and_initialize_enclave(sgx_enclave_id_t *eid) {
  ...
  if (updated == 1) {
    ...
    if (!ofs.good())
      cout << "Warning: Failed to save the launch token to \"" << TOKEN_NAME
           << "\"" << endl;
    else
      ofs << token;
  }
...
void write_o(int i, int size) { memcpy(page->addr + i, &value, size); }
...
int main(int argc, char **argv) {
  ...
  diff = (double)(end.tv_usec - start.tv_usec) / 1000000 +
         (double)(end.tv_sec - start.tv_sec);

  FILE *f = fopen("result_read.txt", "a");
  fprintf(f, "%lf sec\n", diff);
  fclose(f);
