...
TEST_F(TransactionTest, multipleSharedOwners) {
  transactionA_->addLock(row_id_, Lock::LockMode::kShared, lock_);
  transaction_b_->addLock(row_id_, Lock::LockMode::kShared, lock_);

  EXPECT_TRUE(transactionA_->hasLock(row_id_));
...
TEST_F(TransactionTest, noSharedOnExclusive) {
  transactionA_->addLock(row_id_, Lock::LockMode::kExclusive, lock_);
  EXPECT_TRUE(transactionA_->hasLock(row_id_));
...
TEST_F(TransactionTest, entersShrinkingPhase) {
  ...
  transactionA_->releaseLock(row_id_, lock_);
...
TEST_F(TransactionTest, lockBudgetDecreases) {
  ...
  transactionA_->addLock(row_id_, Lock::LockMode::kShared, lock_);
  transactionA_->addLock(row_id_ + 1, Lock::LockMode::kExclusive, another_lock);
